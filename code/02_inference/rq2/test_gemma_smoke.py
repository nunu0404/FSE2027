#!/usr/bin/env python3
"""Smoke test for google/gemma-3-12b-it on GPU 0 with logit extraction."""
...
SNAPSHOT = Path("/ANON/scratch_rq1/hf/hub/models--google--gemma-3-12b-it/snapshots/96b6f1eccf38110c56df3a15bffe176da04bfd80")
VERDICT_RE = re.compile(r"FINAL[_\s-]*VERDICT\s*[:=]\s*(A|B)\b", re.IGNORECASE)

print(f"[SMOKE] Loading Processor from {SNAPSHOT}...")
...
print(f"[SMOKE] Loading Model on cuda:0...")
...
if verdict_index is not None:
    ...
    print("[SMOKE] PASSED 100% agreement check!")
